...
```rust,ignore
use pontifex::attestation::{PcrMeasurement, Verifier};
...
Each configuration must pin **PCR0**, the hash of the whole enclave image. A configuration without
it constrains nothing about the code being run, so it is rejected rather than quietly accepted.
Listing several configurations means "any one of these releases", and each is matched in full, so
values from different releases can never be mixed.
