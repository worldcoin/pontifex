...
fn real_attestation_bytes() -> Vec<u8> {
	let (_, attestation_doc_base64, _, _) = get_real_attestation_document();
	STANDARD
		.decode(attestation_doc_base64)
		.expect("Failed to decode base64")
}
...
fn test_attestation_with_tampered_signature() {
	let (verifier, _, _, _) = get_real_attestation_document();
...
fn test_attestation_with_expired_certificate() {
	...
	let (mut verifier, _, _, _) = get_real_attestation_document();
